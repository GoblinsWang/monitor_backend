...
struct http_response *http_response_new() {
    struct http_response *httpResponse = (http_response *)malloc(sizeof(struct http_response));
    httpResponse->body = NULL;
    httpResponse->statusCode = Unknown;
    httpResponse->statusMessage = NULL;
    httpResponse->response_headers = (response_header *)malloc(sizeof(struct response_header) * INIT_RESPONSE_HEADER_SIZE);
